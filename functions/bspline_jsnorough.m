function [res] = bspline_jsnorough(coefs_0,t_all,sinelv_all,snr_all,knots,bspline_order,...
...
Lk=(2*pi)/Lcar;
Lk = (coefs_0(end)*sin((4*pi*h1(gps).*sinelv_all(gps))/Lcar)+...
    coefs_0(end-1)*cos((4*pi*h1(gps).*sinelv_all(gps))/Lcar));
res=[res;Lk-snr_all(gps)];
end
% GLO
if glo==1
tmpc=tmpc+1;
...
Lcar=Lcar.';
Lk=(2*pi)./Lcar;
Lk = (coefs_0(end-(tmpc-1)*2)*sin((4*pi*h1(glo).*sinelv_all(glo))./Lcar)+...
    coefs_0(end-(tmpc-1)*2-1)*cos((4*pi*h1(glo).*sinelv_all(glo))./Lcar));
res=[res;Lk-snr_all(glo)];
end
% GAL
if gal==1
tmpc=tmpc+1;
...
Lk=(2*pi)/Lcar;
Lk = (coefs_0(end-(tmpc-1)*2)*sin((4*pi*h1(gal).*sinelv_all(gal))/Lcar)+...
    coefs_0(end-(tmpc-1)*2-1)*cos((4*pi*h1(gal).*sinelv_all(gal))/Lcar));
res=[res;Lk-snr_all(gal)];
end
end
